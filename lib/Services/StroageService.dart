...
class StorageService {
  static Future<String> uploadProfilePicture(String url, File imageFile) async {
    // THIS METHOD USE TWO PARAMETER - URL, AND A IMAGE FILE.
    String? uniquePhotoId = Uuid().v4(); // GENERATED A UNIQUE ID.
    File? image = await compressImage(
        uniquePhotoId, imageFile); // CALL A METHOD TO COMPRESS THE IMAGE AND SAVE IT A VARIABLE WHICH IS FILE TYPE.
    ...
    UploadTask uploadTask = storageRef
        ...
        .putFile(image!); //UPLOAD THE FILE IN THE LOCATION ON STROAGR/IMAGES/USERES/FILENAME.JPG
    TaskSnapshot taskSnapshot = await uploadTask.whenComplete(() => null);
    String downloadUrl = await taskSnapshot.ref
        .getDownloadURL(); // TAKE THE LINK(STRING) TO A VARIABLE "downloadUrl" WHEN UPLOAD IS COMPLETED.
  ...
  static Future<String> uploadTweetPicture(File imageFile) async {
    ...
    UploadTask uploadTask = storageRef.child('images/posts/post_$uniquePhotoId.jpg').putFile(image!);
    TaskSnapshot taskSnapshot = await uploadTask.whenComplete(() => null);
    String downloadUrl = await taskSnapshot.ref.getDownloadURL();
    return downloadUrl;
  }
